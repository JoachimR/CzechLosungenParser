<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="LOSUNG">
        ...
        <xsl:element name="Losungen" namespace="http://tempuri.org/FreeXml.xsd">


            <xsl:element name="Datum" namespace="http://tempuri.org/FreeXml.xsd">2013-
                <xsl:if test="string-length(@m) &lt; 2">0</xsl:if>
                <xsl:value-of select="@m"/>-
                <xsl:if test="string-length(@d) &lt; 2">0</xsl:if>
                <xsl:value-of select="@d"/>
            </xsl:element>

            <xsl:element name="Sonntag" namespace="http://tempuri.org/FreeXml.xsd">
    ...
    <xsl:template match="OT">
        <xsl:element name="Losungstext" namespace="http://tempuri.org/FreeXml.xsd">
        ...
        <xsl:element name="Losungsvers" namespace="http://tempuri.org/FreeXml.xsd">
            <xsl:value-of select="SL"/>
        </xsl:element>
    </xsl:template>

    <xsl:template match="NT">
        <xsl:element name="Lehrtext" namespace="http://tempuri.org/FreeXml.xsd">
            <xsl:value-of select="IL"/>
            <xsl:text> </xsl:text>
            <xsl:for-each select="L">
                <xsl:value-of select="."/>
        ...
        <xsl:element name="Lehrtextvers" namespace="http://tempuri.org/FreeXml.xsd">
            <xsl:value-of select="SL"/>
        </xsl:element>
    </xsl:template>

    <xsl:template match="TL">
        <xsl:element name="Wtag" namespace="http://tempuri.org/FreeXml.xsd">
            <xsl:choose>
