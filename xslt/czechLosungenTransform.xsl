<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="LOSUNG">
        ...
        <xsl:element name="Losungen">
            ...
            <xsl:element name="Datum">2022-<xsl:if test="string-length(@m) &lt; 2">0</xsl:if><xsl:value-of select="@m"/>-<xsl:if test="string-length(@d) &lt; 2">0</xsl:if><xsl:value-of select="@d"/></xsl:element>
